...
const OrderForm: React.FC = () => {
  ...
  const canPlaceOrder = (cart.length > 0 || donationAmount > 0) && 
                        ...
                        zelleConfirmation && 
                        zelleChecked && 
                        (deliveryOption === 'Pickup' || (deliveryOption === 'Delivery' && deliveryAddress));

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!canPlaceOrder) {
      alert('Please fill out all required fields, confirm Zelle payment, add Zelle confirmation #, and add items or a donation to your order.');
      return;
    }
    setLastOrderDetails({ deliveryOption });
    placeOrder(customerName, customerContact, deliveryOption, deliveryAddress, zelleConfirmation, isRecurring);
    setCustomerName('');
    setCustomerContact('');
      ...
      {(cart.length > 0 || donationAmount > 0) && (
          <>
            ...
            {deliveryOption === 'Delivery' && cart.length > 0 && (
              <div>
                <label htmlFor="address" className="text-sm font-medium text-brand-brown block mb-2">Delivery Address</label>
                <textarea
                  id="address"
