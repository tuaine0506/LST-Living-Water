...
import { ShoppingCart, ClipboardList, BarChart3, CalendarDays, Lock, Unlock, Droplets, X, BookOpen } from 'lucide-react';
import { useApp } from './context/AppContext';
import ProtectedRoute from './components/ProtectedRoute';

const App: React.FC = () => {
  const { cart, isAdmin, login, logout } = useApp();
  ...
  const navLinkClasses = "flex flex-col items-center justify-center text-center px-1 py-1 text-brand-cream hover:bg-brand-light-green/20 rounded-lg transition-colors duration-200";
    ...
    <HashRouter>
      <div className="font-sans text-gray-800 min-h-screen flex flex-col">
        ...
        <nav className="fixed bottom-0 left-0 right-0 bg-brand-green shadow-top-lg z-20 p-2 border-t border-brand-light-green/20">
          <div className={`container mx-auto grid ${isAdmin ? 'grid-cols-5' : 'grid-cols-1'} gap-1 transition-all duration-300`}>
        ...
        {showLoginModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden border border-brand-light-green">
